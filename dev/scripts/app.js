...
module.exports = "{{each result}}<li>    <img src=\"http://localhost:3000/uploads/{{$value.companyLogo}}\">    <div>        <h2>{{$value.companyName}}</h2>        <p>            <span>                {{$value.positionName}}            </span>            <span>{{$value.positionSalary}}</span>        </p>        <p>{{$value.createTime}}</p>    </div></li>{{/each}}"
...
module.exports = {
    ...
    renderPage: function(index) {
        var $main = $('[class*="swiper-slide"]')
            .eq(index)
            .find(".main");
        switch (index) {
            case 0:
                home.getPosList(function(res) {
                    poslist = template.render(poslistTpl, {
                        result: res
                    });
                    $main.html(indexTpl);
...
module.exports = "<nav>  <div class=\"custom-info\">      <span>          10秒钟定制职位      </span>      <a>          <em class=\"icon\"></em>          <em class=\"text\">去登录</em>      </a>  </div></nav><main id=\"main-scroll\">  <ul>    <li class=\"head\">        <img src=\"http://fe.qfqp5.com:8000/static/images/arrow.png\"/>        <span>下拉刷新...</span>    </li>    <li class=\"foot\">        <img src=\"http://fe.qfqp5.com:8000/static/images/arrow.png\" />        <span>上拉加载...</span>    </li>  </ul></main>"
    ...
    myScroll.on("scrollEnd", function() {
      ...
      if (this.y >= -topSize && this.y < 0) {
        ...
        $.ajax({
          ...
          success: function(res) {
            refreshPoslist = template.render(opt.tpl, res.content.data.page);
      ...
      if (maxY > -topSize && maxY < 0) {
        ...
        $.ajax({
          ...
          success: function(res) {
            loadmorePoslist = template.render(opt.tpl, res.content.data.page);
